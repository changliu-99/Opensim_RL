...
class ProstheticsEnv_Chang(OsimEnv):
    ...
    def compute_reward(self):
        state_desc = self.get_state_desc()
        prev_state_desc = self.get_prev_state_desc()
        reward_hack = state_desc["body_vel"]["pelvis"][0] * 0.01
        reward_hack += 0.01  # small reward for still standing
        reward_hack += min(0, state_desc["body_pos"]["head"][0] - state_desc["body_vel"]["pelvis"][0]) * 0.1  # penalty for head behind pelvis
        # reward_hack -= sum([max(0.0, k - 0.1) for k in [self.state_desc[""], self.current_state[10]]]) * 0.02  # penalty for straight legs
        return self.real_reward+reward_hack
