...
class AtariProcessor(Processor):
    def process_observation(self, observation):
        ...
        for i in observation['body_pos']:
            if i != 'pelvis':
                observation_copy['body_pos'][i][0] = observation['body_pos'][i][0] - observation['body_pos']['pelvis'][0]
                observation_copy['body_pos'][i][2] = observation['body_pos'][i][2] - observation['body_pos']['pelvis'][2]

        observation_copy['joint_pos']['back'][0] = observation['joint_pos']['back'][0] - observation['joint_pos']['ground_pelvis'][0]
        obs = dict_to_list_Chang(observation_copy)
        v = np.array(obs).reshape(158)
        return obs
...
def ProcessObservation(obs): #attempt to correct for the pelvis position
    # print(obs)
...
def build_agent(num_action,observation_shape):
    ...
    agent = DDPGAgent(nb_actions=num_action, actor=actor, critic=critic, critic_action_input=critic_action_input,
                  ...
                  random_process=random_process, processor = AtariProcessor,gamma=.99, target_model_update=1e-3,
                  delta_clip=1.)
...
if args.train:
    ...
    #             observation, r, done, info = env.step(action,project = False)
